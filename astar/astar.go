...
func Astar(board [][]string, startx, starty, endx, endy int) {
	aStar(board, startx, starty, endx, endy)
...
func aStar(board [][]string, startx, starty, endx, endy int) {
	...
	//  初始化优先队列
	pq := &Pqueue{pairWithValue{startx, starty, heuristic(startx, starty, endx, endy)}}
	heap.Init(pq)
	...
	for len(*pq) != 0 {
		...
		for i := 0; i < size; i++ {
			...
			for _, d := range direction {
				...
				visited[dx][dy] = true
				v := heuristic(startx, starty, dx, dy) + heuristic(dx, dy, endx, endy)
